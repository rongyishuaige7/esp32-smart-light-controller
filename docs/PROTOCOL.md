...
公开固件只提交空值的 `firmware/src/wifi_credentials.example.h`。默认不会创建接入点、不会尝试连接 Wi‑Fi，也不会启动 HTTP；此状态没有 HTTP 监听者，不会返回 JSON、401 或 503。需要本机复现实验时，复制它为 Git 忽略的 `firmware/src/wifi_credentials.h` 并填写自己的可信局域网凭据；只有成功连接后，HTTP 服务才监听设备所在局域网的端口 `80`。凭据文件绝不能提交、截图、粘贴到 Issue 或作为构建证据。
...
仅在本机凭据成功连接、HTTP 已启动时返回当前程序的一次本地状态快照。示例仅展示字段格式，不是设备在线、当前读数或真实硬件证据：

...
```text
源码来源已确认；公开候选已完成安全启动、分片 JSON 请求、传感器不可用语义、默认无凭据不开 Wi‑Fi/HTTP、无危险 HTTP 重置接口和依赖锁定整理。固件构建与最终默认分支的 exact-HEAD CI 必须在发布链中单独验证；当前 ESP32、四路 LED、BH1750、NVS、本机凭据连接、HTTP、倒计时和自动模式尚未按当前公开提交重新真机复测。
```
